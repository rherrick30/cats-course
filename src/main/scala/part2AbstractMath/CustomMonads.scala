...
object CustomMonads extends App {
  ...
  implicit object OptionMonad extends Monad[Option] {
    ...
    // tailrecM mustn't stack-overflow
    @tailrec
    override def tailRecM[A, B](a: A)(f: A => Option[Either[A, B]]): Option[B] =
      f(a) match {
        case None => None
        case Some(Left(v)) => tailRecM(v)(f)
        case Some(Right(v2)) => Some(v2)
      }
  }
  ...
  L1     R2   R3     R4
   ...
   tr([R3, 3, 2, 1], [3, 1], [B4]) =
   tr([3, 2, 1], [3, 1], [B3, B4]) =
   tr([2, 1], [1], [B34]) =     // B34 = new branch containing B3 and B4
  ...
  */
  ...
  val newTree = TreeMonad.flatMap(tree)(v=> Branch(Leaf(v + 1 ), Leaf(v + 2)))
  println(newTree)
